...
/datum/disease/appendicitis/stage_act()
	...
	if(istype(affected_mob,/mob/living/carbon/human))
		...
		if(H.species.name == SPECIES_DIONA || H.species.name == SPECIES_IPC || H.species.name == SPECIES_VOX)
			src.cure()

	if(stage == 1)
		if(!affected_mob.internal_organs_by_name[BP_APPENDIX])
	...
	if(stage > 3)
		if(prob(1) && ishuman(affected_mob))
			...
			var/obj/item/organ/external/BP = H.organs_by_name[BP_GROIN]
			BP.sever_artery()
