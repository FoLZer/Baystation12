...
/datum/disease2/effect/zombie/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(ishuman(mob))
		...
		if(infected_organ == null && holder.ticks == 0)
			var/list/organs = list(BP_L_ARM, BP_R_ARM, BP_L_LEG, BP_R_LEG, BP_L_HAND, BP_R_HAND, BP_L_FOOT, BP_R_FOOT) // Organs that you can actually cut off are checked first to give a chance
			organs = shuffle(organs) + shuffle(list(BP_CHEST, BP_GROIN, BP_HEAD))

			for(var/o in organs)
				var/obj/item/organ/external/BP = H.get_organ(o)
				if(BP && !BP_IS_ROBOTIC(BP) && BP.is_usable())
					infected_organ = BP
					break

		if(QDELETED(infected_organ) || !infected_organ || BP_IS_ROBOTIC(infected_organ) || infected_organ.is_stump() || (infected_organ.status & ORGAN_CUT_AWAY))
			disease.dead = TRUE
...
/datum/disease2/effect/heal/starlight/heal(mob/living/carbon/human/M,datum/disease2/disease/disease, actual_power)
	...
	M.heal_organ_damage(heal_amt, heal_amt)
	return 1
...
/datum/disease2/effect/heal/chem/heal(mob/living/carbon/human/M,datum/disease2/disease/disease, actual_power)
	for(var/datum/reagent/R in M.reagents.reagent_list) //Not just toxins!
		M.reagents.remove_reagent(R, actual_power)
		if(prob(2))
...
/datum/disease2/effect/heal/metabolism/heal(mob/living/carbon/human/M,datum/disease2/disease/disease, actual_power)
	if(M.reagents)
		M.handle_chemicals_in_body() //this works even without a liver; it's intentional since the virus is metabolizing by itself
	var/lost_nutrition = 2
	M.nutrition = max(M.nutrition - (lost_nutrition * M.species.metabolism_mod), 0) //Hunger depletes at 2x the normal speed
	if(prob(2))
...
/datum/disease2/effect/heal/darkness/heal(mob/living/carbon/human/M,datum/disease2/disease/disease, actual_power)
	...
	M.heal_organ_damage(heal_amt, heal_amt * 0.5) //more effective on brute
	return 1
...
/datum/disease2/effect/sensory_restoration/activate(mob/living/carbon/M,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(holder.stage	== 4)
		...
		if(ishuman(M))
			...
			var/obj/item/organ/internal/eyes/IO = H.internal_organs_by_name[BP_EYES]
			if(istype(IO))
				if(IO.damage > 0)
	...
	switch(holder.stage)
		...
		if(10,11,12,13)
			if(prob(10) && ishuman(mob))
				var/mob/living/carbon/human/H = mob
				var/organ = pick(list(BP_R_ARM , BP_L_ARM , BP_R_LEG , BP_L_LEG, BP_R_HAND, BP_L_HAND, BP_R_FOOT, BP_L_FOOT))
				var/obj/item/organ/external/BP = H.organs_by_name[organ]
				if (BP && !(BP.is_stump()))
					mob.emote("scream")
...
/datum/disease2/effect/bones/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	switch(holder.stage)
		...
		if(8)
			if(ishuman(mob))
				var/mob/living/carbon/human/H = mob
				var/obj/item/organ/external/BP = pick(H.organs)
...
/datum/disease2/effect/mind/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		...
		if(ishuman(mob) && prob(10))
			var/mob/living/carbon/human/H = mob
			var/obj/item/organ/internal/brain/IO = H.internal_organs_by_name[BP_BRAIN]
			if (IO.damage < IO.min_broken_damage)
				IO.take_general_damage(1)
			mob.adjustBrainLoss(5)
		else
...
/datum/disease2/effect/hallucinations/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		var/message = pick("Something appears in your peripheral vision, then winks out.", "You hear a faint whisper with no source.", "Your head aches.")
		to_chat(mob, "<span class='notice'>[message]</span>")
	else if(prob(20) || holder.stage == 2)
		...
		var/message = pick("Oh, your head...", "Your head pounds.", "They're everywhere! Run!", "Something in the shadows...")
		to_chat(mob, "<span class='userdanger'>[message]</span>")
...
/datum/disease2/effect/giggle/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		...
		if(prob(30))
			var/message = pick("AHAHAHA!","MUST LAUGH", "HELP ME, I CAN'T STOP")
			to_chat(mob, "<span class='userdanger'>[message]</span>")
	...
	if(prob(20) || holder.stage	== 1)
		...
		if(prob(10))
			if(prob(50))
				...
				mob.SetSleeping(max(mob.sleeping, 5 SECONDS))
...
/datum/disease2/effect/hair/proc/is_race_valid(race)
	if(race == SPECIES_HUMAN || race == SPECIES_TAJARA)
		return TRUE
	return FALSE
...
/datum/disease2/effect/hair/proc/shed(mob/living/carbon/human/H, onlyface)
	...
	if(!onlyface)
		...
		else if(H.species.name == SPECIES_TAJARA)
...
/datum/disease2/effect/drool/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		...
		if(prob(30))
			var/message = pick("You drool for a moment, forgetting to close your mouth.", "You can't stop drooling.")
			to_chat(mob, "<span class = 'warning'>[message]</span>")
		else
			mob.emote("drool")
			disease.spread(mob, 1)
...
/datum/disease2/effect/headache/activate(mob/living/carbon/mob,datum/disease2/effectholder/holder,datum/disease2/disease/disease)
	if(ishuman(mob))
		var/mob/living/carbon/human/H = mob
		if(H.species && !H.species.species_flags[SPECIES_FLAG_NO_PAIN])
			if(prob(20) || holder.stage	== 1)
