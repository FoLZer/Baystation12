...
/mob/living/carbon/human/xenomorph/proc/powerc(X, Y)//Y is optional, checks for weed planting. X can be null.
	if(incapacitated())
...
/mob/living/carbon/human/xenomorph/humanoid/verb/plant_xeno()
...
/*
/mob/living/carbon/human/xenomorph/humanoid/verb/ActivateHuggers()
	...
	return
*/
/mob/living/carbon/human/xenomorph/humanoid/verb/whisp(mob/M as mob in oview())
	set name = "Whisper (10)"
...
/mob/living/carbon/human/xenomorph/humanoid/verb/transfer_plasma_xeno(mob/living/carbon/human/xenomorph/M as mob in oview())
...
/mob/living/carbon/human/xenomorph/humanoid/proc/corrosive_acid_xeno(O in oview(1)) //If they right click to corrode, an error will flash if its an invalid target./N
...
/*
/mob/living/carbon/human/xenomorph/humanoid/proc/neurotoxin(mob/target in oview())
...
/mob/living/carbon/human/xenomorph/humanoid/proc/screech()
	set name = "Screech!"
	...
	for(var/mob/living/carbon/human/H in oviewers())
		if(H.sdisabilities & DEAFENED || H.stat == DEAD || istype(H.l_ear, /obj/item/clothing/ears/earmuffs) || istype(H.r_ear, /obj/item/clothing/ears/earmuffs))
			to_chat(H, "<span class='warning'>You feel strong vibrations and quiet noise...</span>")
			continue
		if(H.species.oxy_mod == 0 || H.species.species_flags[SPECIES_FLAG_NO_PAIN]) // so IPCs, dioneae, abductors, skeletons, zombies, shadowlings, golems and vox armalis get less debuffs
...
#define ALIEN_ACID 2
/mob/living/carbon/human/xenomorph/humanoid/proc/toggle_neurotoxin(message = 1)
...
/mob/living/carbon/human/xenomorph/humanoid/proc/neurotoxin_xeno()
...
/mob/living/carbon/human/xenomorph/humanoid/ClickOn(atom/A, params)
	face_atom(A)
...
/mob/living/carbon/human/xenomorph/humanoid/proc/split_neurotoxin(atom/target)
	...
	switch(neurotoxin_on_click)
		if(ALIEN_NEUROTOXIN)
			if(!powerc(50))
				return
			BB = new /obj/item/projectile/energy/neurotoxin(usr.loc)
			adjustToxLoss(-50)
			neurotoxin_next_shot = world.time  + neurotoxin_delay
		if(ALIEN_ACID)
			if(!powerc(150))
				return
			BB = new /obj/item/projectile/energy/alien_acid(usr.loc)
...
#define ALREADY_STRUCTURE_THERE (locate(/obj/structure/alien/air_plant) in get_turf(src))      || (locate(/obj/structure/alien/egg) in get_turf(src)) \
                             || (locate(/obj/machinery/door/unpowered/simple/resin) in get_turf(src))   || (locate(/obj/structure/alien/resin/wall) in get_turf(src)) \
...
/mob/living/carbon/human/xenomorph/humanoid/proc/resin_xeno() // -- TLE
	...
	if(powerc(75, 1))
		...
		playsound(src, 'sound/effects/resin_build.ogg', 100)
		switch(choice)
			if("resin door")
...
/mob/living/carbon/human/xenomorph/humanoid/verb/regurgitate()
	set name = "Regurgitate"
	set desc = "Empties the contents of your stomach."
	set category = "Alien"

	if(powerc())
		var/obj/item/organ/internal/stomach = internal_organs_by_name[BP_STOMACH]
...
/mob/living/carbon/human/xenomorph/humanoid/verb/air_plant()
