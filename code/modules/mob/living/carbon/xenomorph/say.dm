/mob/living/carbon/human/xenomorph/say(message)
	...
	if (length(message) >= 1)
		if (department_radio_keys[copytext(message, 1, 3)] == "alientalk")
			...
			if (!stat)
				playsound(src, pick('sound/voice/xenomorph/talk_1.ogg', 'sound/voice/xenomorph/talk_2.ogg', 'sound/voice/xenomorph/talk_3.ogg', 'sound/voice/xenomorph/talk_4.ogg'), 100, 45) // So aliens can hiss while they hiss yo/N
			return ..("<span class='alien'>[message]</span>", sanitize = 0)

/mob/living/carbon/human/xenomorph/facehugger/say(message)
...
/mob/living/proc/alien_talk(message)
		...
		if(!S.stat)
			if(S.alien_talk_understand)
				if(S.alien_talk_understand == alien_talk_understand)
					S.show_message(rendered, AUDIBLE_MESSAGE)
			else if (S.hivecheck())
				S.show_message(rendered, AUDIBLE_MESSAGE)
	...
	if (length(heard))
		...
		for (var/mob/M in heard)
			M.show_message(rendered, AUDIBLE_MESSAGE)
