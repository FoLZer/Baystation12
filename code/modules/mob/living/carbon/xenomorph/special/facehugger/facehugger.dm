...
/obj/item/clothing/mask/facehugger/Initialize()
	..()
	GLOB.facehuggers_list += src
	return INITIALIZE_HINT_LATELOAD

/obj/item/clothing/mask/facehugger/LateInitialize()
...
/obj/item/clothing/mask/facehugger/proc/show_messageold(message, m_type)
	if(current_hugger)
		var/mob/living/carbon/human/xenomorph/facehugger/FH = current_hugger
...
/obj/item/clothing/mask/facehugger/proc/CanHug(mob/living/carbon/C, check = 1)
	if(!istype(C) || (stat && stat == DEAD) || locate(/obj/item/alien_embryo) in C.contents || istype(C.wear_mask, src) || loc == C)
		return FALSE
	if(check)
		if(isturf(src.loc))
			if(!(C in view(1, src)))
...
/mob/living/carbon/human/proc/mouth_is_protected()
	if(istype(head, /obj/item/clothing/head))
		if((head.item_flags & ITEM_FLAG_AIRTIGHT) && (head.body_parts_covered & FACE))
			return head
	else if(istype(wear_mask, /obj/item/clothing/mask))
		if((wear_mask.item_flags & ITEM_FLAG_AIRTIGHT) && (wear_mask.body_parts_covered & FACE))
			return wear_mask
	return FALSE
...
/obj/item/clothing/mask/facehugger/proc/Impregnate(mob/living/carbon/target, mob/living/carbon/human/xenomorph/facehugger/FH)
	...
	if(!sterile)
		...
		if(current_hugger)
			var/mob/living/carbon/human/xenomorph/larva/new_xeno = new /mob/living/carbon/human/xenomorph/larva(new_embryo)
