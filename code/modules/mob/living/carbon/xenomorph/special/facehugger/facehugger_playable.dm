...
/mob/living/carbon/human/xenomorph/facehugger
...
/mob/living/carbon/human/xenomorph/facehugger/Initialize()
	. = ..()
...
/mob/living/carbon/human/xenomorph/facehugger/start_pulling(atom/movable/AM)
	to_chat(src, "<span class='warning'>You are too small to pull anything.</span>")
	return

/mob/living/carbon/human/xenomorph/facehugger/swap_hand()
	return

/mob/living/carbon/human/xenomorph/facehugger/movement_delay()
	var/tally = 0
	if (istype(src, /mob/living/carbon/human/xenomorph/facehugger)) //just in case
		tally = -1
	return (tally + move_delay_add + config.alien_delay)

/mob/living/carbon/human/xenomorph/facehugger/u_equip(obj/item/W)
	if (W == r_hand)
		r_hand = null
		update_inv_r_hand(0)

/mob/living/carbon/human/xenomorph/facehugger/attack_ui(slot_id)
	return

/mob/living/carbon/human/xenomorph/facehugger/proc/can_grab(atom/movable/target, show_warnings = TRUE)
...
/mob/living/carbon/human/xenomorph/facehugger/proc/Grab(atom/movable/target, show_warnings = TRUE)
 ...
 */
/mob/living/carbon/human/xenomorph/facehugger/proc/leap_at_face(mob/living/carbon/C)
	if(ishuman(C) || istype(C.species, /datum/species/monkey)) // CP! THIS IS DELTA SIX! DO WE NEED THIS? CP!
		var/obj/item/clothing/mask/facehugger/FH = new(loc)
		src.loc = FH
		FH.current_hugger = src
		FH.Attach(C)

/mob/living/carbon/human/xenomorph/facehugger/regenerate_icons()
	update_inv_r_hand(0)
	update_hud()
	update_icons()

/mob/living/carbon/human/xenomorph/facehugger/update_icons()
...
/mob/living/carbon/human/xenomorph/facehugger/update_hud()
	//TODO
	if (client)
...
/obj/item/weapon/larva_bite/Initialize(mapload, mob/victim)
	. = ..()
	chestburster = loc
	affecting = victim

	hud = new /obj/screen/larva_bite(src)
	hud.icon = 'icons/mob/screen1_alien.dmi'
...
/obj/item/weapon/larva_bite/proc/s_click(obj/screen/S)
	...
	if(ishuman(affecting))
		...
		var/obj/item/organ/external/chest/BP = H.organs_by_name[BP_CHEST]
		if((BP.status & ORGAN_BROKEN) || H.stat == DEAD) //I don't know why, but bodyparts can't be broken, when human is dead.
			...
			chestburster.playsound_local(null, 'sound/voice/xenomorph/small_roar.ogg', 100, vary = FALSE, ignore_environment = TRUE)
			H.death()
			...
			H.rupture_lung()
			BP.createwound(CUT, 50, 0)
			qdel(src)
		else
			last_bite = world.time
			playsound(src, 'sound/weapons/bite.ogg', 100)
			H.apply_damage(rand(7, 14), BRUTE, BP_CHEST)
			H.shock_stage = 20
			H.Weaken(1)
			H.emote("scream")
	else if(istype(H.species, /datum/species/monkey))
		if(H.stat == DEAD)
			chestburster.loc = get_turf(H)
			chestburster.visible_message("<span class='danger'>[chestburster] bursts thru [H]'s butt!</span>")
			chestburster.playsound_local(null, 'sound/voice/xenomorph/small_roar.ogg', 100, vary = FALSE, ignore_environment = TRUE)
			qdel(src)
		else
			last_bite = world.time
			H.adjustBruteLoss(rand(35, 65))
			playsound(src, 'sound/weapons/bite.ogg', 100)
			H.Weaken(8)
...
/obj/item/weapon/fh_grab/Initialize(mapload, mob/victim)
	. = ..()
	assailant = loc
	affecting = victim
...
/obj/item/weapon/fh_grab/Process()
	...
	if(state >= GRAB_AGGRESSIVE)
		...
		if(iscarbon(affecting))
			affecting.reagents.add_reagent(/datum/reagent/dexalin, 2)
...
/obj/item/weapon/fh_grab/proc/s_click(obj/screen/S)
	...
	for(var/mob/living/carbon/human/xenomorph/larva/baby in affecting.contents)
		to_chat(assailant, "<span class='warning'>[affecting] already impregnated.</span>")
		qdel(src)
		return
	...
	switch(state)
		if(GRAB_PASSIVE)
			var/mob/living/carbon/human/xenomorph/facehugger/FH = assailant
