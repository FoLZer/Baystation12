...
/proc/get_larva_candidates()
	...
	while(!candidates.len && afk_time <= afk_threesold)
		for(var/mob/observer/G in GLOB.player_list)
			if(!G.client)
				continue
...
/obj/item/alien_embryo/LateInitialize()
	...
	switch(stage)
		...
		if(4)
			...
			if(prob(2))
				...
				to_chat(affected_mob, "<span class='warning'>[message].</span>")
			if(prob(33))
...
/obj/item/alien_embryo/proc/AttemptGrow()
	if(controlled_by_ai)
		...
		if(ishuman(affected_mob)) // we're fucked. no chance to revive a person
			...
			H.rupture_lung()
		var/mob/living/carbon/human/xenomorph/larva/new_xeno = new /mob/living/carbon/human/xenomorph/larva(get_turf(affected_mob))
...
/obj/item/alien_embryo/proc/RefreshInfectionImage()
	for(var/mob/living/carbon/human/xenomorph/alien in GLOB.player_list)
		if(alien.client)
...
/obj/item/alien_embryo/proc/AddInfectionImages(mob/living/C)
	if(C)
		for(var/mob/living/carbon/human/xenomorph/alien in GLOB.alien_list)
			if(alien.client)
