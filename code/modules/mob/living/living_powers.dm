...
/mob/living/proc/zombie_consume()
	...
	src.visible_message("<span class='danger'>[src] яростно вгрызается в голову [target], разрывая его плоть.</span>")
	if(do_mob(src, target, 5 SECONDS))
		to_chat(target,"<span class='danger'>[src] отрывает вашу плоть от костей!</span>")
		to_chat(src,"<span class='danger'>You feed hungrily off \the [target]'s flesh.</span>")
		target.adjustBruteLoss(25)
		if(target.getBruteLoss() < -target.maxHealth)
			target.gib()
