...
/mob/living/simple_animal/special/scp173/Initialize()
	. = ..()
	for(var/mob/living/simple_animal/special/scp173/SA in GLOB.living_mob_list_) //only 1 can exist at the same time
		if(SA != src)
			qdel(SA)
...
/mob/living/simple_animal/special/scp173/Life()
	...
	for(var/mob/living/L in view(7,src))
		...
		if(light_amount <= 3)
			if(prob(max(1,L.scp_mark * 4)))
				src.loc = T
				src.dir = L.dir
				playsound(L, 'sound/effects/blobattack.ogg', 100)
				L.gib()
				did_move = 1
	...
	if(!did_move && turfs_around.len)
		...
		if(!no_where_to_jump)
			...
			playsound(src, 'sound/effects/scp_move.ogg', 100)
